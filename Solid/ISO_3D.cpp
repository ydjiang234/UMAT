...
void test1(double *stress, double *statev, double *ddsdde, double *sse, double *spd, double *scd,
           ...
           double *celent, double *dfgrd0, double *dfgrd1, int *noel, int *npt, int *layer, int *kspt, int *kstep, int *kinc)
{
    double E, anu, para1, G;
    E = props[0];
    anu = props[1];
    para1 = E * anu / (1.0 + anu) /(1.0 - 2.0 * anu);
    G = E / 2.0 / (1.0 + anu);
    ...
    ddsdde[0*(*ntens)+1] = para1;
    ddsdde[0*(*ntens)+2] = para1;
    ddsdde[1*(*ntens)+2] = para1;
    ddsdde[1*(*ntens)+0] = para1;
    ddsdde[2*(*ntens)+0] = para1;
    ddsdde[2*(*ntens)+1] = para1;
